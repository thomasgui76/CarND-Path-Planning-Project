...
1. check the map waypoint file "../data/highway_map.csv", we can find each segment around 30 meters, and the s value start from 0 to maximum 6945.554 meters. the data keeps the road/lanes coordinates(including frenet and Cartesian). actually, we use the data to transform the ego_car(Main) x,y coordinates to s, d.
